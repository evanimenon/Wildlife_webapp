...
def run(
    ...
    # Directories
    save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)  # increment run  #modified it to not increment and update in same folder from general.py
    ...
    if webcam:
        ...
        # import pdb; pdb.set_trace()
        dataset = LoadImages(source, img_size=imgsz, stride=stride, auto=pt, batch_size=batch_size,resume=resume, dir_path=save_dir, empty_path=empty_path)
        # import pdb; pdb.set_trace()
        # bs = 1  # batch_size
        bs=batch_size
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                # Rescale boxes from img_size to im0 size
                det[:, :4] = scale_coords(im.shape[2:], det[:, :4], im0.shape[:2]).round()
                # Print results
                for c in det[:, -1].unique():
                    n = (det[:, -1] == c).sum()  # detections per class
                    cls_res[names[int(c)]] = ((cls_res[names[int(c)]] + int(n)) if names[int(c)] in cls_res else int(n))
                # Write results
